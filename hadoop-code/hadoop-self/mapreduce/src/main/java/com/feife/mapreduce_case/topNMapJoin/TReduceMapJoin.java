 ...
 **/
public class TReduceMapJoin extends Reducer<TKey, IntWritable, Text,IntWritable> {
    ...
    protected void reduce(TKey key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException {
        ...
        while (ite.hasNext()){
            ...
            ite.next();
            if(flg == 0){
                //reduce端虽然不需要操作什么，但是需要个映射的码值给输出
                rKey.set(key.getYear()+"-"+key.getMonth()+"-"+key.getDay() +"-"+key.getJoinId());
                rval.set(key.getWd());
